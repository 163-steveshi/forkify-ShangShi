...
class RecipeView extends View {
  ...
  //function add event handler to this js from model.js's request for changing view when use click or reload
  addHandlerRender(handler) {
    window.addEventListener('hashchange', handler);
    window.addEventListener('load', handler);
  }
  //control the button of add or remove server's number
  addHandlerUpdateServings(handler) {
    this._parentElement.addEventListener('click', function (e) {
      e.preventDefault();
      //select the button that uer click
      const btn = e.target.closest('.btn--tiny');
      if (!btn) return;

      const updateTo = Number(btn.dataset.updateTo);
      if (Number(updateTo) > 0) handler(updateTo); //you can't have 0 servings
    });
  }
